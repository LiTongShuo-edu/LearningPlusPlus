...
| 招聘方希望快速核验的能力 | 建议查看内容 | 已有证据 |
| --- | --- | --- |
| 指针、引用与动态内存基础 | [`代码案例/指针与引用^`](./代码案例/指针与引用^) 与 [`代码案例/内存管理^`](./代码案例/内存管理^) | 两个岗位定向示例纳入自动运行测试 |
| 测试结果整理与失败保留 | [`代码案例/案例-测试结果统计^`](./代码案例/案例-测试结果统计^) 与 [`docs/调试与验证说明.md`](./docs/调试与验证说明.md) | 输出通过率与失败项；新增示例 16/16 测试通过 |
| 构建与持续验证 | [`CMakeLists.txt`](./CMakeLists.txt) 与 [GitHub Actions](https://github.com/LiTongShuo-edu/LearningPlusPlus/actions) | Windows / MSVC 环境自动构建并运行 CTest |
| 代码来源和 AI 协作边界 | [`docs/参考边界与原创声明.md`](./docs/参考边界与原创声明.md) 与 [`docs/AI协作开发记录.md`](./docs/AI协作开发记录.md) | 区分手写旧作、AI 协作复现与岗位扩展示例 |

**当前可核验结果：**13 个 `＊` 复现案例与 3 个 `^` 扩展示例构建成功、16/16 项自动测试通过；34 份早期手写旧作独立编译记录为 33/34 通过，并如实保留遗留错误。
